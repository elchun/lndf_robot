...
class EvaluateRackPlaceGraspIdealStep(EvaluateNetwork):
    ...
    def run_trial(self, iteration: int = 0, obj_scale: float = -1,
        ...
        print('Extents: ', np.max(target_obj_pcd_obs, axis=0) - np.min(target_obj_pcd_obs, axis=0))
        ...
        for i in range(n_place_trials):
            ...
            # time.sleep(1.0)
            teleport_img_fname = osp.join(self.eval_grasp_imgs_dir,
                f'{str(iteration).zfill(3)}_06{i}teleport_place_release.png')
            self._take_image(teleport_img_fname)
